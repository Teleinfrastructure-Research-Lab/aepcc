...
def generate_random_tensors(signal_def, replace_value=4):
    ...
    for key, shape in signal_def.items():
        ...
        if key == "mask" or key =="masks":
            ...
            random_tensors[key] = torch.randn(new_shape)
...
if __name__ == "__main__":
    smol.register(register_loss=False, register_dl=False)
    registry = smol.get_architecture_registry()
    ...
    for name in tqdm(registry, total = len(registry)):
        try:
            ...
            arch = CommonArchitecture(name)
            _output = arch(_input)
            verify_tensor_shapes(_output, output_def)
            check_tensor_validity(_output)
        except Exception as e:
            raise Exception(f"Test on architecture {name} failed: {e}") from e
